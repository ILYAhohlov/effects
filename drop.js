...
// ---------------------------------------------------------------------------
const params = {
  radius: 0.30,        // base size of the blob
  deform: 0.36,        // how far the surface bulges in/out
  frequency: 1.0,      // number of lobes across the surface
  morphSpeed: 0.30,    // how fast the shape changes
  rotSpeed: 0.12,      // how fast the blob tumbles
  specular: 1.0,       // strength of the glints
  shininess: 140,      // tightness of the glints
  glowStrength: 0.70,  // brightness of the outer ring glow
  colorBlue: "#4099FF",
  colorMagenta: "#E633BF",
  glowA: "#33B5FF",
  glowB: "#E24DD0",
  liquidSpeed: 0.50,   // how fast the liquid flows
  liquidScale: 2.20,   // size of the swirls
  liquidBright: 1.00,  // overall interior brightness
  filament: 1.40,      // brightness of the bright threads
  core: 0.30,          // glow at the very centre
  background: "#070A18", // canvas / page background
  blend: "Auto",       // "Auto" | "Glow" (additive) | "Ink" (works on light bg)
};

// ---------------------------------------------------------------------------
// Presets (shown as animated thumbnails on the left)
// ---------------------------------------------------------------------------
// per-orb settings (everything the shader needs); the global blend mode is separate
type Values = Omit<typeof params, "blend">;
...
function blendFor(v: Values): number {
  if (params.blend === "Glow") return 0;
  if (params.blend === "Ink") return 1;
...
function drawOrb(x: number, y: number, w: number, h: number, v: Values, time: number) {
  gl!.viewport(x, y, w, h);
  gl!.scissor(x, y, w, h);
...
const presetEls: HTMLElement[] = [];
const previews: { el: HTMLElement; values: Values }[] = [];
